...
test("example recommends bounded depth 3 via the OpenCode 2 key, not stale depth 2", async () => {
  ...
  try {
    ...
    const example = JSON.parse(await readFile(join(destination, "config", "overlay.example.json"), "utf8"))
    assert.equal(example.experimental.subagent_depth, 3)
    const schema = JSON.parse(await readFile(join(destination, "config", "overlay.schema.json"), "utf8"))
