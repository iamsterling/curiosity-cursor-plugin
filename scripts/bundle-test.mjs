...
const overlay = {
  ...
  permissions: [{ action: "read", resource: "*", effect: "allow" }], plugins: ["test/plugin"], experimental: { subagentDepth: 2 },
}
...
test("reports stale exported assets with a stable code", async () => {
  ...
  try {
    ...
    await writeFile(join(destination, "commands", "verify.md"), "stale", "utf8")
...
test("detects missing assets, bad command references, leakage, and invalid overlay policy", async () => {
  ...
  try {
    await exportBundle(destination, { overlay })
    await rm(join(destination, "skills", "verify", "SKILL.md"))
    await writeFile(join(destination, "commands", "task.md"), "skill: absent-skill\nCrafty", "utf8")
    await writeFile(join(destination, "config", "overlay.json"), JSON.stringify({
      host: { apiVersion: "999" }, defaultAgent: "absent", enabledAgents: ["worker"], models: { worker: "provider/model" }, permissions: [], plugins: ["plugin", "plugin"], experimental: { subagentDepth: 3 }
    }), "utf8")
