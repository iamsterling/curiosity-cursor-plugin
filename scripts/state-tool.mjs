...
import {
  DiagnosticError, attachContract, attestSemanticCompletion, importLegacyV4,
...
async function main() {
  ...
  if (command === "attest-completion") {
    const target = option("--target"), jobId = option("--job")
    const state = await readNativeState(target), index = state.jobs.findIndex((job) => job.id === jobId)
    if (index < 0) throw new Error(`Job not found: ${jobId}`)
    const attestationRef = JSON.parse(await fs.readFile(option("--attestation"), "utf8"))
    state.jobs[index] = await attestSemanticCompletion(state.jobs[index], { authority: "external-loop-evidence", attestationRef }, { directory: option("--directory", false) || process.cwd() })
    await writeNativeState(target, state)
    return print({ status: "attested", contractId: state.jobs[index].contract.contractId })
  }
  throw new Error("Usage: state-tool.mjs import-legacy-v4 --source <v4.json> --target <v1.json> [--apply] | attach-contract --target <v1.json> --job <id> --proposal <proposal.json> [--directory <project>] | attest-completion --target <v1.json> --job <id> --attestation <ref.json> [--directory <project>]")
}
