...
const assets = [
  "config", "skills", "commands", "agents", "src",
  "docs/research", "docs/generic-bundle.md"
]
...
async function validateOverlay(target, diagnostics) {
  const overlayPath = join(target, "config", "overlay.json")
  try {
    ...
    if (!roles.includes(overlay.defaultAgent) || !overlay.enabledAgents?.includes(overlay.defaultAgent)) diagnostics.push(diagnostic("BUNDLE_DEFAULT_AGENT_INVALID", "config/overlay.json", "defaultAgent must be enabled"))
    for (const role of overlay.enabledAgents ?? []) {
      if (!roles.includes(role)) diagnostics.push(diagnostic("BUNDLE_AGENT_UNKNOWN", "config/overlay.json", role))
      if (!providerRoute(overlay.models?.[role])) diagnostics.push(diagnostic("BUNDLE_MODEL_ROUTE_REQUIRED", `config/overlay.json#models.${role}`, "supply a provider-qualified operator-selected route"))
    }
    if (!Array.isArray(overlay.permissions) || !overlay.permissions.length) diagnostics.push(diagnostic("BUNDLE_PERMISSIONS_REQUIRED", "config/overlay.json", "declare at least one permission rule"))
  } catch { diagnostics.push(diagnostic("BUNDLE_OVERLAY_MISSING", "config/overlay.json", "create an installation overlay from the example")) }
}
...
export async function validateBundle(destination) {
  ...
  let commandFiles = []
  try { commandFiles = (await files(join(target, "commands"))).filter((path) => path.endsWith(".md")) }
  ...
  for (const path of commandFiles) {
    ...
    for (const skill of text.matchAll(/^skill:\s*([\w-]+)\s*$/gm)) {
      try { await stat(join(target, "skills", skill[1], "SKILL.md")) }
...
if (import.meta.url === `file://${process.argv[1]}`) {
  if (!destination || !["validate", "export", "doctor"].includes(operation)) {
    console.error("BUNDLE_USAGE: node scripts/bundle-assets.mjs <validate|export|doctor> <isolated-destination>")
    process.exitCode = 2
  } else {
