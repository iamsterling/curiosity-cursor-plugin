...
const healthy = {
  ...
  subagentDepth: 3, hooks: ["tool.execute.before", "tool.execute.after", "event.subscribe"],
  directShellDetected: false, writerState: "healthy", featureIDs: ["hook-foundation"],
...
test("doctor covers compatibility, writers, hooks, features, routes, depth, drift, state, shell, and resources", () => {
  const codes = diagnose({ ...healthy, pluginApiVersion: "bad", writerState: "contended", hooks: [], featureIDs: [], routeIDs: [], subagentDepth: 1, resourceDrift: ["agents/reviewer.json"], stateStatus: "corrupt", directShellDetected: true }).map((item) => item.code)
