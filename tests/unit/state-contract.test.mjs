...
import { compileHandoff } from "../../src/features/handoff/compiler.mjs"
import {
...
} from "../../src/features/loop-compat/state.mjs"
...
const proposal = async () => compileHandoff(JSON.parse(await fs.readFile(path.join(root, "tests/fixtures/handoff/B-behavioral-bug.json"), "utf8")))
...
test("contract lifecycle rejects fake progress, blocked dependencies, missing evidence, and worker completion forgery", async () => {
  ...
  const evidenceFile = path.join(root, "tests/fixtures/native-empty-v1.json")
  ...
  const evidenceRefs = ["red", "green", "command-output"].map((kind) => ({ criterionId: "rejects", kind, locator: "tests/fixtures/native-empty-v1.json", digest }))
...
test("context, retry, compaction, and property mutations produce stable diagnostics", async () => {
  ...
  missingContext.contract.contexts = [{ id: "missing", contextType: "artifact", locator: "tests/fixtures/does-not-exist", digest: `sha256:${"0".repeat(64)}`, required: true }]
  ...
  mismatchedContext.contract.contexts = [{ id: "bad", contextType: "artifact", locator: "tests/fixtures/native-empty-v1.json", digest: `sha256:${"0".repeat(64)}`, required: true }]
