...
import { attachContract, readNativeState, writeNativeState } from "../../src/features/loop-compat/state.mjs"
import { applyRuntimeContractRetry, setGoalComplete, setGoalProgress } from "../../src/features/loop-compat/legacy-runtime.mjs"
...
async function contractJob(directory) {
  const source = JSON.parse(await fs.readFile(path.join(root, "tests/fixtures/handoff/B-behavioral-bug.json"), "utf8"))
...
test("ordinary CLI exposes no semantic attestation command", async () => {
  const source = await fs.readFile(path.join(root, "tools/state-tool.mjs"), "utf8")
