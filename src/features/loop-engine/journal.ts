...
export const decodeLoopJournal = (value: unknown): LoopJournal => {
  ...
  exact(root, [
    "schemaVersion", "revision", "rootSessionID", "claim", "dispatch", "dispatchState", "mode", "budgets",
    "usageBudgetsDisabled", "iteration", "currentIterationID", "promptID", "noProgress", "evidenceCursor",
    "ledgerRevision", "terminalEventIDs", "childSessionIDs", "captureWatermark", "compaction", "breaker", "stopReason",
  ], "$journal");
  const claim = object(root.claim, "$journal.claim");
  exact(claim, ["workID", "token", "revision", "digest"], "$journal.claim");
  text(claim.workID, "$journal.claim.workID"); text(claim.token, "$journal.claim.token");
  integer(claim.revision, "$journal.claim.revision", 1); text(claim.digest, "$journal.claim.digest");
  const dispatch = object(root.dispatch, "$journal.dispatch");
  exact(dispatch, ["id", "digest"], "$journal.dispatch");
  text(dispatch.id, "$journal.dispatch.id"); text(dispatch.digest, "$journal.dispatch.digest");
  ...
  integer(budgets.maxChildren, "$journal.budgets.maxChildren"); integer(budgets.maxTools, "$journal.budgets.maxTools", 1);
  ...
  integer(root.revision, "$journal.revision", 1); text(root.rootSessionID, "$journal.rootSessionID");
  if (!["prepared", "dispatched", "executing", "terminal"].includes(String(root.dispatchState))) fail("$journal.dispatchState");
  ...
  if (root.currentIterationID !== iterationIdentity(String(claim.digest), String(dispatch.digest), iteration)) fail("$journal.currentIterationID");
  ...
  integer(root.evidenceCursor, "$journal.evidenceCursor"); integer(root.ledgerRevision, "$journal.ledgerRevision");
