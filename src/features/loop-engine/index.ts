...
export class NativeLoopEngine {
  ...
  static async open(projectDirectory: string, effects: Effects): Promise<NativeLoopEngine> {
    ...
    return new NativeLoopEngine(root, effects);
  }
  async status(): Promise<LoopJournal> {
    const value = await readJSON(this.journalPath);
    if (!value || (value as { schemaVersion?: number }).schemaVersion !== 1)
      throw new DiagnosticError(value ? "LOOP_VERSION_UNSUPPORTED" : "LOOP_NOT_STARTED");
    return value as LoopJournal;
  }
  async start(input: Pick<LoopJournal, "claim" | "rootSessionID" | "dispatch" | "budgets">): Promise<LoopJournal> {
    ...
    await withLease(this.root, async () => {
      ...
      if (existing && (existing as LoopJournal).mode === "running") throw new DiagnosticError("LOOP_ALREADY_RUNNING");
      ...
      journal = {
        ...
        promptID: `opencode2-loop-${input.claim.workID}-1`,
        noProgress: 0,
        evidenceCursor: 0,
        terminalEventIDs: [],
        childSessionIDs: [],
  ...
  async observeTerminal(input: {
    ...
    descendantsTerminal: boolean;
    toolsTerminal: boolean;
  }): Promise<void> {
    ...
    await withLease(this.root, async () => {
      ...
      if (input.sessionID !== current.rootSessionID || !input.descendantsTerminal || !input.toolsTerminal) {
        const ambiguous = {
          ...
          stopReason: "LOOP_LINEAGE_AMBIGUOUS",
          terminalEventIDs: [...current.terminalEventIDs, input.id],
        };
        await atomicWrite(this.journalPath, `${JSON.stringify(ambiguous)}\n`);
        return;
      }
      const noProgress = input.evidenceCursor > current.evidenceCursor ? 0 : current.noProgress + 1;
      if (noProgress >= current.budgets.maxNoProgress) {
        await atomicWrite(
          this.journalPath,
          `${JSON.stringify({ ...current, revision: current.revision + 1, mode: "stopped", stopReason: "LOOP_NO_PROGRESS_LIMIT", noProgress, terminalEventIDs: [...current.terminalEventIDs, input.id] })}\n`,
        );
        return;
      }
      if (current.iteration >= current.budgets.maxIterations) {
        await atomicWrite(
          this.journalPath,
          `${JSON.stringify({ ...current, revision: current.revision + 1, mode: "stopped", stopReason: "LOOP_ITERATION_LIMIT", terminalEventIDs: [...current.terminalEventIDs, input.id] })}\n`,
        );
        return;
      }
      ...
      nextPrompt = {
        ...
        revision: current.revision + 1,
        iteration,
        currentIterationID: randomUUID(),
        promptID: `opencode2-loop-${current.claim.workID}-${iteration}`,
        noProgress,
        evidenceCursor: input.evidenceCursor,
        terminalEventIDs: [...current.terminalEventIDs, input.id],
      };
      await atomicWrite(this.journalPath, `${JSON.stringify(nextPrompt)}\n`);
    });
  ...
  async observeUserInput(input: { sessionID: string; inputID: string; type: "user" | "synthetic" }): Promise<void> {
    ...
    await withLease(this.root, async () => {
      const current = await this.status();
      if (current.mode === "running" && input.sessionID === current.rootSessionID && input.inputID !== current.promptID)
        await atomicWrite(
          this.journalPath,
          `${JSON.stringify({ ...current, revision: current.revision + 1, mode: "paused", stopReason: "LOOP_USER_INTERRUPTION" })}\n`,
        );
    });
  }
  ...
  private async setMode(mode: LoopJournal["mode"], stopReason: string | undefined): Promise<void> {
    await withLease(this.root, async () => {
      ...
      await atomicWrite(this.journalPath, `${JSON.stringify(next)}\n`);
    });
  }
}
