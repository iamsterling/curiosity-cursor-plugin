...
interface Effects {
  prompt(input: { sessionID: string; text: string; id: string; metadata: Record<string, string>; resume: true }): Promise<unknown>;
  interrupt(input: { sessionID: string }): Promise<unknown>;
  validateContinuation?(input: { claim: LoopJournal["claim"]; journalRevision: number }): Promise<ContinuationAuthority>;
  reconcileDispatch?(input: { sessionID: string; promptID: string; iterationID: string }): Promise<DispatchState | "unknown">;
}
...
const promptFor = (journal: LoopJournal): Parameters<Effects["prompt"]>[0] => ({
  ...
  id: journal.promptID,
  metadata: { "opencode2-config": "native-loop-v1", iteration: String(journal.iteration), claim: journal.claim.digest, causation: journal.currentIterationID },
  resume: true,
});
...
export class NativeLoopEngine {
  ...
  async start(input: Pick<LoopJournal, "claim" | "rootSessionID" | "dispatch" | "budgets">): Promise<LoopJournal> {
    ...
    await withLease(this.root, async () => {
      ...
      if (existing && ["running", "stopping"].includes(decodeLoopJournal(existing).mode)) throw new DiagnosticError("LOOP_ALREADY_RUNNING");
      journal = {
        schemaVersion: 1, revision: 1, ...input, dispatchState: "prepared", mode: "running", usageBudgetsDisabled: true,
        iteration: 1, ...iterationFields(input.claim, input.dispatch, 1), noProgress: 0, evidenceCursor: 0,
        ledgerRevision: input.claim.revision, terminalEventIDs: [], childSessionIDs: [], captureWatermark: 0,
  ...
  async markExecuting(promptID: string): Promise<void> {
    await this.update((current) => current.mode === "running" && current.promptID === promptID && current.dispatchState === "dispatched"
      ? { ...current, revision: current.revision + 1, dispatchState: "executing" } : current);
  }
  ...
  async observeTerminal(input: TerminalObservation): Promise<void> {
    ...
    await withLease(this.root, async () => {
      ...
      if (current.mode === "stopping") {
        ...
        await this.write({ ...current, revision: current.revision + 1, dispatchState: "terminal", mode: "stopped", stopReason: "LOOP_INTERRUPTED", terminalEventIDs: [...current.terminalEventIDs, input.id] });
        return;
      }
      if (current.mode !== "running") return;
      const terminal = { ...current, revision: current.revision + 1, dispatchState: "terminal" as const, terminalEventIDs: [...current.terminalEventIDs, input.id] };
      ...
      const stopReason = breaker.repeatedFailures >= current.budgets.maxNoProgress ? "LOOP_REPEATED_FAILURE_LIMIT"
        : breaker.repeatedActions >= current.budgets.maxNoProgress ? "LOOP_REPEATED_ACTION_LIMIT"
        : noProgress >= current.budgets.maxNoProgress ? "LOOP_NO_PROGRESS_LIMIT"
        : current.iteration >= current.budgets.maxIterations ? "LOOP_ITERATION_LIMIT" : undefined;
      if (stopReason) { await this.write({ ...terminal, mode: "stopped", stopReason, noProgress, breaker }); return; }
      const rejection = await this.continuationRejection(current, input);
      if (rejection) { await this.write({ ...terminal, mode: "ambiguous", stopReason: rejection }); return; }
      const iteration = current.iteration + 1;
      nextPrompt = { ...terminal, dispatchState: "prepared", iteration, ...iterationFields(current.claim, current.dispatch, iteration),
        noProgress, evidenceCursor: input.evidenceCursor, ledgerRevision: input.ledgerRevision ?? current.ledgerRevision,
        captureWatermark: input.captureWatermark ?? current.captureWatermark, breaker };
  ...
  async observeUserInput(input: { sessionID: string; inputID: string; type: "user" | "synthetic" }): Promise<void> {
    ...
    await this.update((current) => current.mode === "running" && input.sessionID === current.rootSessionID && input.inputID !== current.promptID
      ? { ...current, revision: current.revision + 1, mode: "paused", stopReason: "LOOP_USER_INTERRUPTION" } : current);
  }
  async prepareCompaction(references: readonly string[], watermark: number): Promise<void> {
    if (!references.length || new Set(references).size !== references.length) throw new DiagnosticError("LOOP_COMPACTION_REFERENCES_REQUIRED");
    await this.update((current) => ({ ...current, revision: current.revision + 1,
      compaction: { state: "prepared", references: [...references].sort(), beforeWatermark: watermark, afterWatermark: 0 } }));
  }
  async completeCompaction(references: readonly string[], afterWatermark: number): Promise<void> {
    await this.update((current) => {
      if (current.compaction.state !== "prepared" || JSON.stringify([...references].sort()) !== JSON.stringify(current.compaction.references)
        || afterWatermark < current.compaction.beforeWatermark)
        return { ...current, revision: current.revision + 1, mode: "ambiguous", stopReason: "LOOP_COMPACTION_CONTINUITY_AMBIGUOUS" };
  ...
  async resume(): Promise<void> {
    ...
    const prepared: LoopJournal = { ...withoutReason, revision: current.revision + 1, mode: "running", dispatchState: "prepared" };
  ...
  private async continuationRejection(current: LoopJournal, input: TerminalObservation): Promise<string | undefined> {
    if (current.dispatchState !== "dispatched" && current.dispatchState !== "executing") return "LOOP_ROOT_STATE_AMBIGUOUS";
    if (input.sessionID !== current.rootSessionID || !input.descendantsTerminal || !input.toolsTerminal || input.lineageProven !== true)
    ...
    if (current.compaction.state === "prepared" || (current.compaction.state === "complete"
    ...
    const advanced = input.ledgerAdvanceAccepted === true && input.ledgerRevision !== undefined
      && input.ledgerRevision > current.ledgerRevision && input.evidenceCursor > current.evidenceCursor;
  ...
  private nextBreaker(current: LoopJournal, input: TerminalObservation): LoopJournal["breaker"] {
    const repeatedFailures = input.failureSignature ? (input.failureSignature === current.breaker.lastFailure ? current.breaker.repeatedFailures + 1 : 1) : 0;
    const repeatedActions = input.actionSignature ? (input.actionSignature === current.breaker.lastAction ? current.breaker.repeatedActions + 1 : 1) : 0;
    return { ...(input.failureSignature ? { lastFailure: input.failureSignature } : {}), repeatedFailures,
      ...(input.actionSignature ? { lastAction: input.actionSignature } : {}), repeatedActions };
  }
  private async dispatchPrepared(journal: LoopJournal): Promise<void> {
    try {
      await this.effects.prompt(promptFor(journal));
      await this.update((current) => current.promptID === journal.promptID && current.dispatchState === "prepared"
        ? { ...current, revision: current.revision + 1, dispatchState: "dispatched" } : current);
    } catch (error) { await this.setMode("ambiguous", "LOOP_DISPATCH_AMBIGUOUS"); throw error; }
  }
  private async reconcileRestart(): Promise<void> {
    ...
    if (!this.effects.reconcileDispatch) {
      await this.write({ ...current, revision: current.revision + 1, mode: "ambiguous", stopReason: "LOOP_RESTART_OUTCOME_AMBIGUOUS" });
      return;
    }
    const observed = await this.effects.reconcileDispatch({ sessionID: current.rootSessionID, promptID: current.promptID, iterationID: current.currentIterationID });
    if (observed === "unknown" || (current.dispatchState !== "prepared" && observed !== current.dispatchState)) {
      await this.write({ ...current, revision: current.revision + 1, mode: "ambiguous", stopReason: "LOOP_RESTART_OUTCOME_AMBIGUOUS" });
      return;
    }
  ...
  private async setMode(mode: LoopJournal["mode"], stopReason: string | undefined): Promise<void> {
    await this.update((current) => {
      const { stopReason: _stopReason, ...withoutReason } = current;
      return { ...withoutReason, revision: current.revision + 1, mode, ...(stopReason === undefined ? {} : { stopReason }) };
    });
  }
  private async update(change: (current: LoopJournal) => LoopJournal): Promise<void> {
    await withLease(this.root, async () => { const current = await this.status(); const next = change(current); if (next !== current) await this.write(next); });
  }
  private async write(journal: LoopJournal): Promise<void> { decodeLoopJournal(journal); await atomicWrite(this.journalPath, `${JSON.stringify(journal)}\n`); }
}
