...
async function buildGoalPrompt(directory, job) {
  const sections = []
  sections.push(`Working directory:\n${path.resolve(directory)}\nKeep every file operation inside this directory. Prefer workspace-relative paths such as \"src/features/loop-compat/legacy-runtime.mjs\"; never turn a relative path into a root path such as \"/src/features/loop-compat/legacy-runtime.mjs\".`)
...
// port uses the V2 plugin API surface through the composition root, ctx.session.* actions,
...
export async function registerLegacyLoopCompatibility(ctx) {
