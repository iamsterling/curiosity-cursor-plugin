...
Explicit invocation names include `/curiosity-coordinator`, `/curiosity-worker`, `/curiosity-implementer`, `/curiosity-researcher`, `/curiosity-reviewer`, `/curiosity-strategist`, and `/curiosity-engineering <action>`; automatic selection is nondeterministic. The workflow requires user-selected Plan Mode and explicit plan acceptance before edits. Restoration uses Cursor-owned Plan/Todo/session/returned Task context and asks on ambiguity. The inert stop hook performs zero automatic follow-ups, powers no continuation, and its limit of five is only an upper bound. Cursor documents `readonly: true` as no file edits and no state-changing shell commands. It is not confidentiality, no-read, local-only processing, no-network/no-MCP guarantee, or proof of prompt compliance; behavior is version-, mode-, tool-policy-, and admin-policy-dependent. Invocation-scoped Cursor CLI live smokes ran in disposable repositories. The editor runtime remains unverified because Accessibility blocked testing; live behavior remains version-, model-, and mode-dependent.
...
- no Cursor installation, marketplace publication, or conversion beyond six native agents, one workflow skill, and one inert stop hook is claimed. Static prompt validation and bounded CLI smokes are implementation evidence; editor behavior and complete runtime parity remain unverified.
