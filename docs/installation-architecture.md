...
Explicit invocation names are `/curiosity-coordinator`, `/curiosity-researcher`, `/curiosity-reviewer`, and `/curiosity-strategist`; automatic selection is nondeterministic. Cursor documents `readonly: true` as no file edits and no state-changing shell commands. It is not confidentiality, no-read, local-only processing, no-network/no-MCP guarantee, or proof of prompt compliance; behavior is version-, mode-, tool-policy-, and admin-policy-dependent. No live Cursor/model smoke test was run.
