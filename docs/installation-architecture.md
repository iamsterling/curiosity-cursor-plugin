...
Explicit invocation names include `/curiosity-coordinator`, `/curiosity-worker`, `/curiosity-implementer`, `/curiosity-researcher`, `/curiosity-reviewer`, `/curiosity-strategist`, and `/curiosity-engineering <action>`; automatic selection is nondeterministic. The workflow requires user-selected Plan Mode and explicit plan acceptance before edits. Reconstruction uses Cursor-owned Plan/Todos/Task context/source/evidence and asks on ambiguity. The stateless command-hook mesh provides bounded guidance and permission decisions but no restoration, continuation, state, transcript reading, or MCP control. Cursor documents `readonly: true` as no file edits and no state-changing shell commands. It is not confidentiality, no-read, local-only processing, no-network/no-MCP guarantee, or proof of prompt compliance; behavior is version-, mode-, tool-policy-, and admin-policy-dependent. Historical invocation-scoped Cursor CLI live smokes ran in disposable repositories. No live model smoke ran for v0.4.0; the editor runtime remains unverified because Accessibility blocked testing, and live behavior remains version-, model-, and mode-dependent.
...
- no Cursor installation, marketplace publication, or conversion beyond six native agents, one workflow skill, and the six-event stateless command-hook mesh is claimed. Static validation is v0.4.0 implementation evidence; historical bounded CLI smokes do not validate this mesh, and editor behavior and complete runtime parity remain unverified.
