...
Explicit invocation names are `/curiosity-coordinator`, `/curiosity-researcher`, `/curiosity-reviewer`, `/curiosity-strategist`, and `/curiosity-engineering`; automatic selection is nondeterministic. The workflow requires user-selected Plan Mode and explicit plan acceptance before edits. Manual continuation is `/curiosity-engineering continue the accepted plan`; the inert stop hook performs zero automatic follow-ups, and its limit of five is only an upper bound. Cursor documents `readonly: true` as no file edits and no state-changing shell commands. It is not confidentiality, no-read, local-only processing, no-network/no-MCP guarantee, or proof of prompt compliance; behavior is version-, mode-, tool-policy-, and admin-policy-dependent. No live Cursor/model smoke test was run.
...
- no Cursor installation, marketplace publication, or conversion beyond the four read-only native agents, one workflow skill, and one inert stop hook is claimed.
