...
The static bundle declares three read-only agents and one writable implementer. The required semantic invariant keeps main orchestration-only; desired host enforcement is unavailable because Cursor children inherit the parent's mode/tool envelope. Use Agent mode for writable hierarchy. Ask/Plan does not provide child write elevation.
