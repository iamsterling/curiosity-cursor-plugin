...
The configured event allowlist is exactly `sessionStart`, `subagentStart`, `beforeShellExecution`, `beforeReadFile`, `postToolUse` matched to `Shell`, and `preCompact`. Each command binds its own event as the sole argument after the shared dispatcher path, for example `node "${CURSOR_PLUGIN_ROOT}/hooks/curiosity-dispatch.mjs" beforeShellExecution`, with a five-second timeout. `subagentStart`, `beforeShellExecution`, and `beforeReadFile` are fail-closed. The three guidance events are fail-open. Stop, subagentStop, generic preToolUse, MCP, prompt, thought/response, specialized after, Tab, sessionEnd, and workspaceOpen hooks are absent.

The trusted static command argument, not stdin, is the event authority and failure-posture authority. The dispatcher allowlists exactly the six configured argument values; a missing, unknown, or additional argument emits `{}` and never dispatches. It accepts at most 256 KiB on stdin, writes exactly one JSON object and a newline to stdout, writes nothing to stderr, and exits zero. Successfully parsed input must be a root object with exactly one decoded `hook_event_name`, and that value must equal the bound argument. Escaped keys and values are valid when they decode to that exact match. Missing, mismatched, duplicate, nested, or contradictory discriminators and nonobject input are malformed. Empty, malformed JSON, oversized input, and all of those malformed discriminator forms deny for a protected bound event and emit `{}` for a guidance bound event, without inspecting raw input to infer posture.
...
1. Config contains exactly the six allowed events, one command path with the matching event argument on each definition, five-second timeouts, the specified fail posture, and only the `Shell` post matcher.
