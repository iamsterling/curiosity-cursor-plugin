...
**Current (2026-08-15).** `iamsterling.curiosity-cursor-plugin` is a research-phase OpenCode plugin foundation for a possible future Cursor-focused conversion. One `Plugin.define` composes completed bundled-agent routing with generic redacted tool/event capture. Capture state is project-scoped under `.opencode/curiosity-cursor-plugin/capture/v1/`.
...
Native Cursor APIs, command mapping, packaging, and installation have not been designed or implemented. Conversion research is pending.
