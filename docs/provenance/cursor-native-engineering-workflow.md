...
No live Cursor/model smoke was run for the original 0.3.0 slice. A separate authorized CLI smoke informed 0.3.1. Later live CLI evidence established a sharper host limitation: Cursor marked an evidence Todo completed and rendered `To-do All done` after its mandatory full suite exited 1. Version 0.3.2 therefore treats native Todos only as attempted-work/progress projections and adds a separate prompt-level Verification Gate over raw requirement, scenario, and command results. The checked-in fixtures prove the static projection for PASS, FAIL, MISSING, and contradictory `All done`; they do not reproduce the live model run or provide runtime authority. No sanitized smoke report is added before retesting. Static/model-mediated guidance cannot prevent host rendering or prove compliance. Transcript parsing remains prohibited, the inert hook is unchanged, and no state/store/runtime is introduced.
