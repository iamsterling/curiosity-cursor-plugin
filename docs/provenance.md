...
The Cursor-native engineering workflow skill, historical inert hook, current v0.4.0 stateless hook mesh, source basis, and runtime unknowns are recorded in [`docs/provenance/cursor-native-engineering-workflow.md`](provenance/cursor-native-engineering-workflow.md). Independently refetched official hook/plugin documentation URLs and response-byte digests are in [`provenance/cursor/hook-mesh-official-docs.json`](../provenance/cursor/hook-mesh-official-docs.json). This custom specification is not an OpenSpec import or implementation.
