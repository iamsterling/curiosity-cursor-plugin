# Design: Align the verification route

## Context
Main orchestrates only and cannot emulate specialists or mutate project, workspace, or temporary paths.

## Decision
`/curiosity-verify` SHALL use exactly one implementer in verification-only mode followed by a fresh reviewer. Routing failure is distinct from authority, owner decision, and evidence failure.
