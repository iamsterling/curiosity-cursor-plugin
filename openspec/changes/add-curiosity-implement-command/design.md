# Design: Add the approved implementation route

## Context
Main orchestrates only and cannot emulate specialists or mutate project, workspace, or temporary paths.

## Decision
`/curiosity-implement` SHALL route an approved change through exactly one implementer and a fresh reviewer. Routing failure is distinct from authority, owner decision, and evidence failure.
