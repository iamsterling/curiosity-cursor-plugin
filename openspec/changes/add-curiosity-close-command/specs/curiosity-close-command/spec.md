...
### Requirement: Add the package close route
`/curiosity-close` SHALL route archive persistence through exactly one implementer and fresh review.

#### Scenario: Required route
- **Given** a completed package is approved for archive
- **When** the command classifies and routes the intent
- **Then** the route validates, archives approved paths, and reviews the result
