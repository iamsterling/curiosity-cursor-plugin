...
### Requirement: Align the feature route
`/curiosity-feature` SHALL route one approved writer and require an owner gate for consequential choices.

#### Scenario: Required route
- **Given** a consequential feature choice is unresolved
- **When** the command classifies and routes the intent
- **Then** the route returns USER_DECISION_REQUIRED before writing
