# Design: Align the feature route

## Context
Main orchestrates only and cannot emulate specialists or mutate project, workspace, or temporary paths.

## Decision
`/curiosity-feature` SHALL route one approved writer and require an owner gate for consequential choices. Routing failure is distinct from authority, owner decision, and evidence failure.
