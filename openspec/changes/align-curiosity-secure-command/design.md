# Design: Align the security route

## Context
Main orchestrates only and cannot emulate specialists or mutate project, workspace, or temporary paths.

## Decision
`/curiosity-secure` SHALL separate routine reviewer, strategist, authorized researcher, and approved fix branches. Routing failure is distinct from authority, owner decision, and evidence failure.
