# Design: Align the bug route

## Context
Main orchestrates only and cannot emulate specialists or mutate project, workspace, or temporary paths.

## Decision
`/curiosity-bug` SHALL route exactly one implementer and a fresh reviewer. Routing failure is distinct from authority, owner decision, and evidence failure.
