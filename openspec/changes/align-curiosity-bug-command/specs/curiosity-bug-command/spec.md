...
### Requirement: Align the bug route
`/curiosity-bug` SHALL route exactly one implementer and a fresh reviewer.

#### Scenario: Required route
- **Given** a reproducible bug is accepted
- **When** the command classifies and routes the intent
- **Then** the route records RED, GREEN, VERIFY, and review evidence
