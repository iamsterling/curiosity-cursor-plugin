# Design: Add the durable ledger route

## Context
Main orchestrates only and cannot emulate specialists or mutate project, workspace, or temporary paths.

## Decision
`/curiosity-ledger` SHALL route SHOW read-only and persistence through exactly one implementer plus fresh review. Routing failure is distinct from authority, owner decision, and evidence failure.
