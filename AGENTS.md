...
- The native Cursor surface is limited to a private locally loaded manifest, four read-only advisors, two bounded writable subagents, one seven-action `/curiosity-engineering` skill, and the stateless v0.4.0 command-hook mesh; never claim broader conversion, guaranteed delegation, installation, or cutover.
...
- The hook mesh must remain one stateless command dispatcher with exactly the six events and mixed failure posture in `docs/specs/cursor-hook-mesh.md`. It owns no plugin state/store/logging/network/transcript parsing/scheduler/daemon/MCP and provides no continuation or completion authority.
