...
export const runRealHostSuite = async () => {
  ...
  try {
    ...
    const records = await waitFor(async () => { const values = await markers(marker); return values.filter((x) => x.kind === "registration").length === 2 ? values : undefined }, 10_000)
    await delay(100)
    ...
    if (all.filter((x) => x.kind === "setup").length !== 1 || all.filter((x) => x.kind === "cleanup").length !== 1 || registrations.length !== 2 || JSON.stringify([...tools].sort()) !== JSON.stringify(TOOL_IDS)) throw new Error("REAL_HOST_REGISTRATION_MISMATCH")
