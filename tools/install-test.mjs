...
const expectedPackageSpec = `@iamsterling/curiosity-cursor-plugin@${packageVersion}`
const temporaryRoot = await fs.mkdtemp(path.join(os.tmpdir(), "curiosity-cursor-plugin-installer-"))
...
try {
  ...
  assert.match(helpResult.stdout, /Curiosity Cursor Plugin installer/)
  ...
  assert.equal(await exists(path.join(local, "plugins", "curiosity-cursor-plugin.js")), true)
  ...
  assert.equal(await exists(path.join(local, "curiosity-cursor-plugin-bundle", "config", "agents", "orchestrator.json")), true)
  assert.equal(await exists(path.join(local, "curiosity-cursor-plugin-bundle", "config", "overlay.schema.json")), true)
  await fs.symlink(path.join(root, "node_modules"), path.join(local, "node_modules"), "dir")
  await import(path.join(local, "plugins", "curiosity-cursor-plugin.js"))
  ...
  await fs.writeFile(path.join(configured, "opencode.json"), JSON.stringify({ plugin: ["@iamsterling/curiosity-cursor-plugin@latest"] }), "utf8")
  await fs.writeFile(path.join(configured, "plugins", "curiosity-cursor-plugin.js"), "duplicate", "utf8")
  await fs.writeFile(path.join(configured, "plugins", "curiosity-cursor-plugin.js"), "duplicate", "utf8")
  ...
  assert.equal(await exists(path.join(configured, "plugins", "curiosity-cursor-plugin.js")), false)
  assert.equal(await exists(path.join(configured, "plugins", "curiosity-cursor-plugin.js")), false)
  ...
  await fs.writeFile(path.join(jsonc, "opencode.jsonc"), `{
    ...
    "plugin": [
      "other-plugin",
      "@iamsterling/curiosity-cursor-plugin",
    ],
  }`, "utf8")
  const jsoncResult = await runInstaller(jsonc)
  assert.equal(jsoncResult.code, 0, jsoncResult.stderr)
  assert.equal(await exists(path.join(jsonc, "plugins", "curiosity-cursor-plugin.js")), false)
  ...
  await fs.writeFile(path.join(lookalike, "opencode.json"), JSON.stringify({ plugin: ["@iamsterling/curiosity-cursor-plugin-extra"] }), "utf8")
  const lookalikeResult = await runInstaller(lookalike)
  assert.equal(lookalikeResult.code, 0, lookalikeResult.stderr)
  assert.equal(await exists(path.join(lookalike, "plugins", "curiosity-cursor-plugin.js")), true)

  console.log("Curiosity Cursor Plugin installer test passed")
} finally {
