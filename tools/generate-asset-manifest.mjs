...
for(const file of files){
 ...
 entries.push({id,kind,sourcePath,installDestination:category,dependencies,owningFeature:compatibility?"loop-compat":id==="handoff-compiler"||id==="compile-handoff"?"handoff":"platform",status:compatibility?"compatibility-deprecated":"active",digest:`sha256:${createHash("sha256").update(text).digest("hex")}`})
}
